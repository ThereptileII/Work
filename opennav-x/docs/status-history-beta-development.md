# Historical status through Beta development

This preserves the incremental status before final Beta qualification. Pending and
rejected-candidate statements below describe those earlier revisions, not the
current packaged build. See [current status](status.md) and
[Beta 1 acceptance](evidence/beta1-a3e6e08-accepted.json) for the exact accepted release.
