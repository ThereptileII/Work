# OpenNav X Beta 1 — Windows test guide

**Beta evaluation only. Not approved for navigation.** Use a desktop PC first.
...
Beta Setup supports only the exact executable hash listed in `BUILD_INFO.md`.
...
1. Download the `OpenNavX-Beta1-Windows` GitHub Actions artifact and extract
   that outer archive. It contains the portable ZIP, Setup, this guide and hashes.
   Find `OpenNavX-Beta1-Portable-win64.zip` and check its entry in
   `SHA256SUMS.txt` if desired (`Get-FileHash` in PowerShell).
2. Extract the entire ZIP to a short, writable path, for example
   `%USERPROFILE%\XNavBeta`. Do not run files inside the ZIP viewer.
...
Beta; never run two copies against one profile.

## 3. Install beside normal OpenCPN

Run `OpenNavX-Beta1-Setup.exe`. An unsigned Beta may trigger Windows reputation
...
Choose **Install** for a first installation, or **Update** for an existing Alpha. Preflight must show a supported configuration; an unsupported
build must fail without changing the installation. Setup stages and verifies
OpenNav under `%LOCALAPPDATA%\OpenNavXAlpha1`, then uses the existing **OpenNav X Alpha 1** Start-menu folder.
This historical folder name and installation directory are deliberately retained
for Alpha upgrades; the application and Apps entry identify Beta 1.
...
For desktop testing, use the **DEMO manual autopilot** only. Enable its simulator,
try AUTO, ±1/±10 and STANDBY, and inspect feedback confirmation and timeout state.
Live control defaults OFF. Beta includes an explicitly bound ST4000 translator
adapter over bidirectional OpenCPN TCP / Actisense complete-PGN ASCII; do not
connect or enable it as part of desktop testing. Physical commands require the
separate boat commissioning procedure. TRACK/WIND remain unavailable. SmartNav
has no steering path. Saved permission never automatically enables a session.
...
- **Update:** run a newer Beta Setup and choose Update. Confirm user data,
...
Beta retains modified/custom files, unpublished staging and logs under
...
Use **Menu → Field diagnostic bundle → Export Diagnostic Bundle** for a bounded
ZIP containing build/recovery state, source health, energy assumptions, adapter
status and recent advisory/transition records. Default export excludes position,
route names, AIS identity, credentials and device identifiers. A recording is
included only after explicit selection and consent; review it before sharing.
Send that bundle with Windows version, DPI/GPU, exact steps, expected/actual
result and screenshots. Raw upstream logs can contain private navigation and
connection information; do not send your complete profile or chart collection.

## 11. Commissioning, recordings and calibration — desktop rehearsal

Open **Menu → Commissioning & recordings**. Check per-item units, source/PGN,
instance, selected priority, observation age and LIVE/AGING/STALE/INVALID state.
In Demo, start a bounded recording, change a failure scenario, stop, and replay
it in the disconnected portable profile. Confirm the conspicuous **REPLAY**
indicator, historical ages and disabled autopilot commands. Return to Demo/live
explicitly. Replay does not move OpenCPN's real ownship or edit its route.

Exercise calibration export from a selected recording. Choose the speed reference
(STW or SOG), power basis and device identity deliberately. The CSV contains
observations to review; it does not automatically install a curve. Do not import
Demo measurements as boat calibration. See `recording-replay-contract.md` and
`energy-model.md` for format, bounded retention and assumptions.
