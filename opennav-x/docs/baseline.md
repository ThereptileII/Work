...
3.5 for older bundled dependencies. It does not bundle docs, coastline or tide
resources. Local wxGTK webview is disabled because that optional dependency is
...
| Legacy actions | `gui/src/toolbar.cpp`, `canvas_menu.cpp`, `options.cpp`, `connections_dlg.cpp` |
...
- Startup policy contract: passed locally in Release and native Windows MSVC CI.
- Pristine Linux build/test: running; final outcome to be recorded.
- Pristine Windows build/test: running after gettext-path correction.
- Windows baseline capture/visual review: pending.
- Real charts, route edit/persistence, AIS, connections and plugin regressions:
  pending; no current claims of preservation from a compile alone.
- XNav runtime, mode switching, installer and hardware integration: not implemented.
