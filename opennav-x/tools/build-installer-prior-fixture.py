#!/usr/bin/env python3
"""Fetch the exact accepted Alpha installer for a real Alpha-to-Beta upgrade.
...
lock = json.loads((root / 'tools/accepted-alpha.lock.json').read_text())
...
if not token:
    raise SystemExit('Read-only Actions token required for the accepted Alpha fixture')
...
assert not metadata['expired'], 'Accepted Alpha artifact expired; restore exact hash-pinned archive'
...
try:
    ...
    if redirect.code != 302:
        raise SystemExit('Accepted Alpha archive request failed: HTTP ' + str(redirect.code)) from None
...
archive = output / 'accepted-alpha.zip'
with urllib.request.urlopen(location, timeout=120) as response, archive.open('wb') as target:
    ...
    while block := response.read(1024 * 1024):
        remaining -= len(block)
        if remaining < 0:
            raise SystemExit('Accepted Alpha archive exceeds its bound')
        target.write(block)
...
(root / 'evidence/local/installer-prior-release.json').write_text(json.dumps({
    ...
    'scope': 'Exact accepted Alpha installer; subsequent real upgrade and rollback gates required'
}, indent=2) + '\n')
print('Exact accepted Alpha 1 installer verified; candidate executable and source untouched')
